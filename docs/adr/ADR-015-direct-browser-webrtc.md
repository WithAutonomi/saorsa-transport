...
The browser protocol is unreleased. Each plaintext frame is a JSON object
immediately followed by its raw binary body, with no inner length prefix. The shared
...
This revises the unreleased format in place; the protocol name and DataChannel
label retain their existing v5 identifiers.
